...
// Resend is optional for local development. Email routes report a delivery
// failure when it is not configured, without preventing the API from starting.
const resend = process.env.RESEND_API_KEY
  ? new Resend(process.env.RESEND_API_KEY)
  : null;

// Use the persistent deployment volume when supplied, otherwise keep uploads
// inside this project so the server works on a local machine as well.
...
app.get("/api/blogs", async (req, res) => {
  try {
    ...
    let sql = `
      SELECT id, title, slug, excerpt, coverImage, author, 
             category, tags, readingTime, published, created_at 
      FROM blogs 
